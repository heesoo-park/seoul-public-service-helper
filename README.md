...
👍 초기에 명확한 기획과 지킬 수 있으면서 명료한 규칙이 개발을 진행하는데 있어서 윤활유 역할을 한다는 것   
👍 주기적인 회의를 통해 소통하며 현재 상황에 대한 점검을 꾸준히 하는 방법
 
🎇 팀장으로서의 역할   
👍 전체적인 개발일정 관리하는 방법(예시: WBS)   
👍 팀원 역할 분배 및 케어하는 것이 어렵지만 팀장으로써 가장 많이 채겨야하는 부분임을 배움   

🎇 기획 - 개발 - 배포 - 유지보수의 사이클   
👍 이전까지는 기획, 개발만 해봤지만 이번에는 한 사이클을 다 해보면서 앱 개발을 시작하고 매듭짓는 흐름을 배움

🎇 Clean Architecture의 구조와 흐름을 알고 이를 앱에 적용하는 방법   
👍 해당 구조를 통해 프로젝트 앱의 유지보수성을 높이고 의존성을 낮출 수 있었음   
👍 깔끔하게 앱에 적용시키기 위해서는 패키지를 잘 관리하고 팀원 모두가 이해하고 있어야함

🎇 NoSQL DB인 Firestore를 활용하여 데이터를 저장하고 읽고 쿼리하는 방법   
👍 앱에 Firestore를 적용시키는 방법을 알게 됨   
👍 하위 컬렉션 구조를 사용하여 데이터베이스 구조를 설정하는 방법을 알게 됨

🎇 Naver Map API를 이용하는 방법   
👍 Naver Map API는 정말 간단하게 사용할 수 있는 것이 장점   
👍 FusedLocationSource를 이용하여 위치 오차가 적은 것도 장점

🎇 Room을 구성하고 쿼리를 만드는 방법   
👍 쿼리를 사용하다보니 SQL문을 좀 더 공부해야할 필요성을 느낌

🎇 Coil을 이용하여 현재 상태에 따라 sacleType과 이미지를 변경하는 방법   
👍 특정 라이브러리에 대한 깊은 탐색으로 인해 알게 됐는데 다른 라이브러리를 가져와 사용할 때 좀 더 깊게 파고들어봐야겠음
